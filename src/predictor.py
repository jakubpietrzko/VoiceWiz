...
class MelSpectrogramDataset(Dataset):
    def __init__(self, mels_goal):
        self.mels_goal = mels_goal
        self.filenames = [f for f in os.listdir(mels_goal) if f.endswith('.pt')][400:1400]

    def __len__(self):
        return len(self.filenames)

    def __getitem__(self, idx):
        audio_file_path = os.path.join(self.mels_goal, self.filenames[idx])
...
class Predictor(nn.Module):
    ...
    def forward(self, mel1, mel2):
        ...
        mel1 = (mel1 - mel1.mean()) / mel1.std()
        mel2 = (mel2 - mel2.mean()) / mel2.std()
    ...
    def validation_on_batch(self, batch_y):
        ...
        print("syf")
        return 0
    def select_random_segments(self, samples):
        segment_length = 150
        segments = []
    ...
    def prepare_dataset_goal(self, mels_goal):
        dataset = MelSpectrogramDataset(mels_goal)
    ...
    def train_model(self, epochs, patience):
        ...
        dataloader = self.prepare_dataset_goal('..\\data\\mels\\')
...
if __name__ == "__main__":
    ...
    model.evaluate_model('..\\data\\mels\\')
